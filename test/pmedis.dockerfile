...
RUN wget https://github.com/redis/redis/archive/refs/tags/6.2.6.tar.gz && rm -rf redis-6.2.6 && tar zxf 6.2.6.tar.gz && cd redis-6.2.6 && make -j`nproc` && cd ..
...
COPY start_pmedis.sh pmedis.conf /workdir/
